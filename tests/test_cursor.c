...
void test_cursor_full_scan() {
  ...
  assert(rows_read ==
         20); // We should have successfully scanned exactly 20 rows!
...
void test_cursor_table_find() {
  printf("Running test_cursor_table_find...\n");

  Table *table = db_open_table("test_cursor_find.db");
  ...
  r.id = 10;
  FixedLengthRowStrategy.serialize(&r, buffer);
  table_insert_row(table, r.id, buffer, row_size);
  r.id = 20;
  FixedLengthRowStrategy.serialize(&r, buffer);
  table_insert_row(table, r.id, buffer, row_size);
  r.id = 30;
  ...
  // Find 20
  Cursor *cursor = table_find(table, 20);
  assert(cursor->end_of_table == false);
  assert(cursor->page_num == 0);
  assert(cursor->slot_num ==
         1); // since it's the second element in sorted array

  // Fetch to ensure we got the right data
  cursor_get(cursor, buffer, row_size);
  Row out_r;
  FixedLengthRowStrategy.deserialize(buffer, &out_r);
  assert(out_r.id == 20);

  cursor_free(cursor);
  db_close_table(table);
  remove("test_cursor_find.db");
  printf("test_cursor_table_find passed!\n");
}
