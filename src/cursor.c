...
void cursor_get(Cursor* cursor, void* dest_buffer, uint32_t row_size) {
  ...
  bool success = table_read_row(cursor->table, cursor->page_num, cursor->slot_num, dest_buffer, row_size);
  if (!success) {
...
void cursor_advance(Cursor* cursor) {
  ...
  cursor->slot_num++;
  if (cursor->slot_num >= table_get_row_count_for_page(cursor->table, cursor->page_num)) {
      cursor->page_num++;
      cursor->slot_num = 0;
